...
def test_cli_w_adult_and_one_arg(runner):
    result = runner.invoke(cli.main, ['-a', 'Ali'])
    assert result.exit_code == 0
    assert not result.exception
    assert result.output.split('\n', 1)[0].strip() == 'https://torrentz2.eu/search?f=Ali&safe=0'
...
def test_cli_w_verfied_option_one_arg(runner):
    ...
    assert not result.exception
    assert result.exit_code == 0
    assert result.output.split('\n', 1)[0].strip() == 'https://torrentz2.eu/verified?f=Ali&safe=1'
...
def test_cli_wo_option_one_arg(runner):
    ...
    assert result.exit_code == 0
    assert not result.exception
    assert result.output.split('\n', 1)[0].strip() == 'https://torrentz2.eu/search?f=Ali&safe=1'
...
def test_cli_with_sort_by_date_and_one_arg(runner):
    ...
    assert result_date.output.split('\n', 1)[0].strip() == 'https://torrentz2.eu/searchA?f=Ali&safe=1'
...
def test_cli_with_sort_by_size_and_one_arg(runner):
    ...
    assert result_size.output.split('\n', 1)[0].strip() == 'https://torrentz2.eu/searchS?f=Ali&safe=1'
...
def test_cli_with_sort_by_peers_and_one_arg(runner):
    ...
    assert result_peers.output.split('\n', 1)[0].strip() == 'https://torrentz2.eu/searchP?f=Ali&safe=1'
...
def test_cli_with_sort_by_ranking_and_one_arg(runner):
    ...
    assert result_ranking.output.split('\n', 1)[0].strip() == 'https://torrentz2.eu/searchN?f=Ali&safe=1'
